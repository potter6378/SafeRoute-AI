...
def _add_cors_headers(response):
    """
    대시보드(민주님 화면)가 이 데몬과 다른 포트/도메인에서 뜰 가능성이 높아서
    미리 열어둔다. 브라우저가 기본적으로 다른 출처(origin) 요청을 막기 때문에,
    이게 없으면 대시보드 쪽 개발자도구 콘솔에 CORS 에러만 찍히고 원인을 알기 어렵다.
    """
...
def list_cases():
    """
    대시보드 '판정 로그' 화면용. 최신순 정렬 + 필터링 + 페이지네이션.
    예: /api/cases?decision=WARN&limit=20

    응답 필드는 saferoute_policy의 make_output()(명세서 6장 "판단→시각화
    전달 형식")을 그대로 따른다 — 이 API가 별도로 필드를 재정의하지 않는다.
    """
    ...
    items = []
    for case in _store.cases.values():
        if decision and case.result.decision.value != decision:
            continue
        if risk_level and case.det.risk_level.value != risk_level:
            continue
        if domain and case.det.domain != domain:
            continue
        items.append(make_output(case).to_dict())

    items.sort(key=lambda x: x["timestamp_decided"], reverse=True)
    total = len(items)
    return jsonify({"total": total, "items": items[offset: offset + limit]}), 200


@app.get("/api/cases/<log_id>")
def get_case(log_id):
    """대시보드에서 로그 한 건 클릭했을 때 상세보기용"""
    case = _store.cases.get(log_id)
    if not case:
        return jsonify({"error": "not found"}), 404
    return jsonify(make_output(case).to_dict()), 200


@app.get("/api/stats/summary")
def stats_summary():
    """대시보드 '통계 요약' 화면용 (파이차트/막대그래프 등에 바로 쓸 수 있는 집계값)"""
    by_decision, by_risk, by_type, domain_counter = {}, {}, {}, {}
    for case in _store.cases.values():
        d = case.result.decision.value
        by_decision[d] = by_decision.get(d, 0) + 1
        r = case.det.risk_level.value
        by_risk[r] = by_risk.get(r, 0) + 1
        t = case.det.detected_type.value
        by_type[t] = by_type.get(t, 0) + 1
        domain_counter[case.det.domain] = domain_counter.get(case.det.domain, 0) + 1

    top_domains = sorted(domain_counter.items(), key=lambda x: -x[1])[:5]
    return jsonify({
        "total_cases": len(_store.cases),
        "by_decision": by_decision,
        "by_risk_level": by_risk,
        "by_detected_type": by_type,
